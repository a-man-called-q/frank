...
describe("Frank control panel", () => {
  ...
  it("keeps the typed Tauri bridge calls explicit", async () => {
    ...
    expect(bridge).toHaveBeenNthCalledWith(2, "set_active_level", { level: "full" });
    expect(bridge).toHaveBeenNthCalledWith(3, "update_settings", { patch: { close_to_tray: false } });
    expect(bridge).toHaveBeenNthCalledWith(4, "prepare_target_change", { targetId: "claude-code", operation: "install" });
    expect(bridge).toHaveBeenNthCalledWith(5, "apply_prepared_plan", { planId: "plan-1" });
    expect(bridge).toHaveBeenNthCalledWith(6, "prepare_pack_change", { operation: { kind: "use", selector: "local@1.0.0" } });
    expect(bridge).toHaveBeenNthCalledWith(7, "apply_prepared_pack", { planId: "pack-plan" });
  });
});
