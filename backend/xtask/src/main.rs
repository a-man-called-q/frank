...
/// Assert every workspace member uses the inherited version rather than
/// hard-pinning its own. The Flutter app has its own client version because
/// it is a separate package and is checked by its pubspec/toolchain docs.
fn version_check(root: &Path) -> Result<()> {
    ...
    let members_dir = root.join("backend/crates");
...
fn repo_root() -> Result<PathBuf> {
    ...
    Ok(backend_dir
        .parent()
        .context("backend has no repository parent directory")?
        .to_path_buf())
}
...
fn architecture_check(root: &Path) -> Result<()> {
    let packages = [
        ...
        (
            "frank-orchestrator",
            "backend/crates/frank-orchestrator/Cargo.toml",
        ),
        ("frank-pack", "backend/crates/frank-pack/Cargo.toml"),
        ("frank-protocol", "backend/crates/frank-protocol/Cargo.toml"),
        ("frank-runner", "backend/crates/frank-runner/Cargo.toml"),
        ("frank-safeio", "backend/crates/frank-safeio/Cargo.toml"),
        ("frank-server", "backend/crates/frank-server/Cargo.toml"),
        ("frank-service", "backend/crates/frank-service/Cargo.toml"),
        ("frank-store", "backend/crates/frank-store/Cargo.toml"),
        ("frank-update", "backend/crates/frank-update/Cargo.toml"),
        ("frank-updater", "backend/crates/frank-updater/Cargo.toml"),
        ("frank-state", "backend/crates/frank-state/Cargo.toml"),
        ("frank-target", "backend/crates/frank-target/Cargo.toml"),
        (
            "frank-toolchain",
            "backend/crates/frank-toolchain/Cargo.toml",
        ),
        (
            "frank-release-cli",
            "backend/crates/frank-release-cli/Cargo.toml",
        ),
        ("xtask", "backend/xtask/Cargo.toml"),
    ];
...
fn expected_architecture_dependencies(package: &str) -> Option<&'static [&'static str]> {
    Some(match package {
        ...
        "frank-protocol" => &[],
        "frank-runner" => &["frank-protocol", "frank-toolchain"],
        "frank-tool-catalog" => &[],
        "frank-safeio" => &[],
        ...
        "frank-store" => &["frank-protocol", "frank-safeio"],
        "frank-target" => &["frank-pack", "frank-safeio"],
        "frank-toolchain" => &["frank-protocol"],
        "frank-release-cli" => &["frank-update"],
        "frank-update" => &[],
        "frank-updater" => &["frank-update"],
...
fn frank_dependencies(manifest: &toml::Value) -> BTreeSet<&str> {
    ...
    for (section, value) in table {
        ...
        for (name, spec) in entries {
            if !name.starts_with("frank-") {
                continue;
            }
            // Optional legacy adapters are not part of a shipped binary's
            // dependency boundary.  They stay available for explicit fixture
            // consumers without making the v1 graph claim a GUI→app edge.
            let optional = spec
                .as_table()
...
fn dist(root: &Path, requested_target: Option<&str>) -> Result<()> {
    ...
    if !release_bin.exists() {
        anyhow::bail!(
            "release binary not found at {} — run `cargo build --release -p frank-cli -p frank-server -p frank-updater` first",
            release_bin.display()
        );
    }
    ...
    let daemon_name = if triple.contains("windows") {
        "frankd.exe"
    } else {
        "frankd"
    };
    let mut release_binaries = vec![binary_name];
    if release_dir.join(daemon_name).is_file() {
        release_binaries.push(daemon_name);
    }
    let updater_name = if triple.contains("windows") {
        "frank-updater.exe"
    } else {
        "frank-updater"
    };
    if release_dir.join(updater_name).is_file() {
        release_binaries.push(updater_name);
    }

    let status = if triple.contains("windows") {
        package_zip_many(&release_dir, &release_binaries, &archive_path)?
    } else {
        package_tar_gz_many(&release_dir, &release_binaries, &archive_path)?
    };
    if !status.success() {
...
fn package_tar_gz(
    ...
    package_tar_gz_many(release_dir, &[binary_name], archive_path)
}

fn package_tar_gz_many(
    release_dir: &Path,
    binaries: &[&str],
    archive_path: &Path,
) -> Result<std::process::ExitStatus> {
...
fn package_zip(binary_path: &Path, archive_path: &Path) -> Result<std::process::ExitStatus> {
    let release_dir = binary_path.parent().unwrap_or_else(|| Path::new("."));
    let binary_name = binary_path
        .file_name()
        .and_then(|name| name.to_str())
        .unwrap_or("frank.exe");
    package_zip_many(release_dir, &[binary_name], archive_path)
}
...
fn package_zip_many(
    ...
    let paths = binaries
        .iter()
        .map(|binary| powershell_quote(&release_dir.join(binary)))
        .collect::<Vec<_>>()
        .join(",");
    let script = format!(
...
"#,
    ...
    fn architecture_policy_rejects_a_forbidden_dependency_edge() {
        ...
        let packages = [
            (
                "frank-app",
                "backend/crates/frank-app/Cargo.toml",
                &[
                    "frank-ledger",
                    "frank-pack",
                    "frank-safeio",
            ...
            (
                "frank-cli",
                "backend/crates/frank-cli/Cargo.toml",
                &[
                    "frank-app",
                    "frank-compress",
                    "frank-ledger",
            ...
            (
                "frank-compress",
                "backend/crates/frank-compress/Cargo.toml",
                &["frank-safeio"][..],
            ),
            (
                "frank-ledger",
                "backend/crates/frank-ledger/Cargo.toml",
                &["frank-pack", "frank-safeio", "frank-state"][..],
            ),
            (
                "frank-mcp",
                "backend/crates/frank-mcp/Cargo.toml",
                &["frank-compress"][..],
            ),
            (
                "frank-pack",
                "backend/crates/frank-pack/Cargo.toml",
                &["frank-safeio"][..],
            ),
            (
                "frank-safeio",
                "backend/crates/frank-safeio/Cargo.toml",
                &[][..],
            ),
            (
                "frank-state",
                "backend/crates/frank-state/Cargo.toml",
                &["frank-pack", "frank-safeio"][..],
            ),
            (
                "frank-target",
                "backend/crates/frank-target/Cargo.toml",
                &["frank-pack", "frank-safeio"][..],
            ),
            (
                "xtask",
                "backend/xtask/Cargo.toml",
        ...
        let forbidden = root.path().join("backend/crates/frank-pack/Cargo.toml");
        fs::OpenOptions::new()
            .append(true)
            .open(forbidden)
            .unwrap()
...
"#,
    ...
    fn version_check_and_checksum_generation_are_fail_closed() {
        ...
        .unwrap();
        let crates_dir = root.path().join("backend/crates");
        fs::create_dir_all(crates_dir.join("frank-cli")).unwrap();
        fs::write(
            crates_dir.join("frank-cli/Cargo.toml"),
