...
fn worktree_operation_matches_task(operation: &OperationView, task_id: TaskId) -> bool {
...
fn operation_lock_resource(operation: &OperationView) -> String {
...
fn is_merge_conflict_error(message: &str) -> bool {
...
pub struct Scheduler {
    ...
    running_by_provider: HashMap<Provider, usize>,
    providers_by_task: HashMap<TaskId, Provider>,
}
...
fn authorize(command: &Command, actor_kind: ActorKind, role: DeviceRole) -> Result<()> {
...
fn validate_supervisor_proposal(
    snapshot: &Snapshot,
    mission_id: MissionId,
...
fn policy_satisfies(profile: &AgentPolicy, requirement: &AgentPolicy) -> bool {
...
fn parse_supervisor_json(text: &str, mission_id: MissionId) -> Option<SupervisorPlanProposal> {
...
fn apply_settings_patch(settings: &mut ServerSettings, patch: SettingsPatch) -> Result<()> {
...
fn validate_project_spec(spec: &ProjectSpec, settings: &ServerSettings) -> Result<()> {
...
fn validate_task_spec(spec: &TaskSpec, tasks: &[TaskView]) -> Result<()> {
...
fn validate_task_view(candidate: &TaskView, tasks: &[TaskView]) -> Result<()> {
...
fn apply_agent_patch(agent: &mut AgentView, patch: AgentPatch) {
...
fn valid_agent_text(value: &str, max_bytes: usize) -> bool {
    !value.trim().is_empty()
        && value.len() <= max_bytes
        && !value.chars().any(|character| character.is_control())
}

fn valid_content_text(value: &str, max_bytes: usize) -> bool {
    !value.trim().is_empty() && value.len() <= max_bytes && !value.contains('\0')
}

fn valid_optional_agent_text(value: Option<&str>, max_bytes: usize) -> bool {
    value.is_none_or(|value| {
        value.len() <= max_bytes && !value.chars().any(|character| character.is_control())
    })
}

fn apply_task_patch(task: &mut TaskView, patch: TaskPatch) {
...
fn is_allowed_path(path: &str, roots: &[String]) -> bool {
...
fn stable_task_commit_message(task_id: TaskId, title: &str) -> String {
    let title = title
        .chars()
        .filter(|character| !character.is_control())
        .collect::<String>();
...
fn now_plus_seconds(seconds: u64) -> u128 {
...
fn local_server_url(settings: &ServerSettings) -> Option<String> {
    if settings.port == 0 {
        return None;
    }
...
fn hash_capability(token: &str) -> String {
    let mut digest = Sha256::new();
