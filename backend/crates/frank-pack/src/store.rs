...
//! lockfile.
//!
//! Remote sources are intentionally outside the v1 trust boundary. A future
//! downloader needs an explicit signature, proxy, and certificate policy; the
//! CLI rejects URLs instead of silently treating them as local paths.
...
impl PackStore {
    ...
    pub fn set_active(&self, active: Option<PackRef>) -> StoreResult<()> {
        ...
        if let Some(ref selected) = active
            && !lock.packs.iter().any(|p| p.pack_ref() == *selected)
        {
