...
impl FrankPaths {
    pub fn from_process() -> Self {
        ...
        let data_root = data_root_base(&home).join("frank");
    ...
    /// Frank 1.0 deliberately uses a versioned root.  The legacy 0.2.x
    /// files remain at their old locations and are never migrated or deleted.
    pub fn v1_data_root(&self) -> PathBuf {
        self.data_root.join("v1")
    }

    pub fn v1_config_root(&self) -> PathBuf {
        self.user_config_dir.join("v1")
    }

    pub fn server_database_path(&self) -> PathBuf {
        self.v1_data_root().join("frank.sqlite3")
    }

    pub fn server_audit_path(&self) -> PathBuf {
        self.v1_data_root().join("audit.jsonl")
    }
...
fn data_root_base(home: &std::path::Path) -> PathBuf {
    #[cfg(target_os = "macos")]
    {
        // Keep XDG_DATA_HOME as an explicit override.  Besides being useful
        // for portable installs, this makes the path deterministic for CLI
        // callers and test sandboxes.  The platform default remains the
        // native Application Support directory.
