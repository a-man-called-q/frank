...
enum Command {
    ...
    Checksums,
    /// Sign canonical update manifest bytes with an offline PKCS#8 Ed25519 key.
    SignManifest {
        #[arg(long)]
        manifest: PathBuf,
        #[arg(long)]
    ...
    /// Derive the public key value used by release builds from an offline
    /// PKCS#8 Ed25519 private key.
    PublicKey {
        #[arg(long)]
        private_key: PathBuf,
    },
    /// Verify a detached Frank update signature before publishing a release.
    VerifyManifest {
        #[arg(long)]
        manifest: PathBuf,
        #[arg(long)]
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::SignManifest {
            ...
            let signature = frank_update::sign_manifest(&bytes, &key)
        ...
        Command::PublicKey { private_key } => {
            let key = fs::read(private_key)?;
            let public_key = frank_update::public_key_base64_from_pkcs8(&key)
                .map_err(|error| anyhow::anyhow!(error.to_string()))?;
            println!("{public_key}");
            Ok(())
        }
        Command::VerifyManifest {
            ...
            let key = base64::engine::general_purpose::STANDARD
                .decode(public_key)
                .map_err(|error| anyhow::anyhow!(error.to_string()))?;
            frank_update::parse_verified_manifest(&bytes, &signature, &key)
                .map_err(|error| anyhow::anyhow!(error.to_string()))?;
            println!("verified");
            Ok(())
        }
...
/// The workspace root is, definitionally, wherever `[workspace]` lives. The
/// Flutter client is a separate package and does not participate in Cargo
/// release metadata.
fn repo_root() -> Result<PathBuf> {
    if let Ok(manifest_dir) = std::env::var("CARGO_MANIFEST_DIR") {
        let path = PathBuf::from(manifest_dir);
        if let Some(root) = path.parent().and_then(Path::parent)
            && is_workspace_root(&root.join("Cargo.toml"))
        {
    ...
    I: IntoIterator<Item = std::io::Result<fs::DirEntry>>,
{
    ...
    // Frank's v1 workspace keeps Rust packages under `backend/`, while the
    // legacy layout used a root-level `crates/` directory. Keep the release
    // tool tolerant of both layouts so isolated fixtures and older checkouts
    // can still be verified and bumped safely.
    let crates_dir = [root.join("backend/crates"), root.join("crates")]
        .into_iter()
    ...
    for candidate in [
        root.join("backend/xtask/Cargo.toml"),
...
fn checksums_from_entries(
    ...
    for entry in directory_entries {
        ...
        if path.is_file()
            ...
            && matches!(ext, "gz" | "zip" | "dmg" | "msi" | "deb" | "rpm")
        {
            entries.push(path);
        }
    }
