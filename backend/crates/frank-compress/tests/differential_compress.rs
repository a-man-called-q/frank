...
fn run_js_oracle(input: &str) -> Option<String> {
    let root = repo_root();
    let helper = root.join("backend/crates/frank-compress/tests/fixtures/run_js_compress.js");
...
fn matches_legacy_js_compressor_on_real_compress_fixtures() {
    let root = repo_root();
    let fixtures_dir = root.join("backend/crates/frank-compress/tests/fixtures/caveman-compress");
